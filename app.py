...
DATABASE = os.path.join(BASE_DIR, 'instance', 'Flavorscope.db')

DEFAULT_CITY = "Bangalore City"  # Default delivery range city
...
# --- Auto-fix missing 'items' column (in case old DB exists) ---
try:
    ...
    if 'items' not in columns:
        ...
        conn.commit()
        app.logger.info("✅ Added missing 'items' column to orders table.")
    conn.close()
except Exception as e:
    app.logger.error(f"❌ Error ensuring 'items' column exists: {e}")

# ---------------------- MAIL CONFIG ----------------------
app.config['MAIL_SERVER'] = 'smtp.gmail.com'
app.config['MAIL_PORT'] = 465
app.config['MAIL_USE_SSL'] = True
app.config['MAIL_USERNAME'] = 'flavorscope@gmail.com'
app.config['MAIL_PASSWORD'] = 'oiya zlhv irvc yowz'
...
def confirm_order():
    ...
    if "Bangalore City" not in location:
    ...
    try:
        ...
        # Save in Orders_Analysis
        customerName = session.get('username', 'Guest')
        items = items_str.split(",")
        for item in items:
            match = re.match(r"(.*) \(x(\d+)\)", item.strip())
            if match:
                ...
                for _ in range(int(quantity)):
                    cursor.execute("INSERT INTO Orders_Analysis (customerName, orderId, productName, orderDate) VALUES (?, ?, ?, CURRENT_DATE)",
                                   (customerName, order_id, item_name))
            else:
                cursor.execute("INSERT INTO Orders_Analysis (customerName, orderId, productName, orderDate) VALUES (?, ?, ?, CURRENT_DATE)",
                               (customerName, order_id, item.strip()))
        conn.commit()
        conn.close()
...
def order_summary():
    ...
    '''
    order = cursor.execute(query, (name,)).fetchone()
    conn.close()
    order_dict = dict(order) if order else None
    return render_template('ordersummary.html', order=order_dict)
